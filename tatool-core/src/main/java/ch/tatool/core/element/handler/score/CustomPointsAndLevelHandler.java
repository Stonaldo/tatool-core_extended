 ...
 * @author Andre Locher
 */
public class CustomPointsAndLevelHandler extends AbstractPointsAndLevelHandler {
	...
	public CustomPointsAndLevelHandler() {
		super("level-handler");
	}
			...
			if (trialOutcome != null
				...
				if (trialCounter >= (sampleSize - 1)) {
					...
					if (performance >= maxThreshold) {
						...
						// level down counter
					} else if (performance <= minThreshold && oldLevel > -2) {
						newLevel = changeLevel(context, oldLevel, -1);
					}
